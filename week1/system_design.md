...
- Бизнес-цель - увеличить конверсии в заказ, средний чек и частоту покупок путем релевантных персонализированных рекомендаций. 
- Почему станет лучше, чем сейчас, от использования ML - увеличение конверсии, среднего чека и частоты покупок ведет к увеличению прибыли, снизить средний срок хранения товара на складе. Также это решение упростит выбор покупателя в приложении, при достаточно хороших рекомендациях выбор будет осуществляться быстрее, при этом товары будут действительно нравиться покупателю.
- Что будем считать успехом итерации с точки зрения бизнеса - модель удовлетворяет минимальным требованиям по метрикам качества.  

#### 1.2. Бизнес-требования и ограничения  

- Краткое описание БТ - система по введенному идентификатору покупателя должна вернуть ранжированный список рекомендаций, рекомендации должны обновляться ежедневно.
...
- Описание бизнес-процесса пилота, насколько это возможно - как именно мы будем использовать модель в существующем бизнес-процессе? - модель будет встроена в существующую экосистему продукта для улучшения показателей продаж и удовлетворенности покупателей. Раз в сутки (уточнить) необходимо для каждого пользователя с помощью модели подготавливать список рекомендаций и отображать в приложении. При покупке какого либо из рекомендованных товаров, не являющихся часто приобретаемым, его карточку необходимо исключать из списка рекомендаций.
- Что считаем успешным пилотом? Критерии успеха и возможные пути развития проекта - достаточная доля (уточнить) рекомендованных товаров будет приобретаться пользователем, без возврата и с выставлением хорошей оценки/отзыва.
...
- Что делаем с технической точки зрения: рекомендательная система, поиск аномалий, прогноз, оптимизация, и др. - Для решения задачи будет разработана рекомендательная система со встроенной моделью ранжирования результата на основе прошлых покупок пользователя.

#### 2.2. Блок-схема решения  

- [Блок-схема](https://github.com/AnnchenK/recsys/blob/main/week1/scheme_recsys.drawio.png) для бейзлайна и основного MVP с ключевыми этапами решения задачи: подготовка данных, построение прогнозных моделей, оптимизация, тестирование, закрытие технического долга, подготовка пилота, другое. `Data Scientist`  
